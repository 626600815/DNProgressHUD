...
#define DNScreen [UIScreen mainScreen].bounds.size
...
@interface DNRefreshButton : UIButton
...
@implementation DNRefreshButton
...
@property (nonatomic, strong) DNRefreshButton *refreshButton;
...
static DNRefreshButton *refreshButtonKey = (DNRefreshButton *)@"refreshButtonKey";
...
- (DNRefreshButton *)refreshButton {
    return objc_getAssociatedObject(self, &refreshButtonKey);
}

- (void)setRefreshButton:(DNRefreshButton *)refreshButton {
...
+ (void)tipView:(NSString *)message toView:(UIView *)view {
    ...
    CGFloat labelWidth = [self widthWithFont:label.font constrainedToHeight:20 text:message] + 40.0;
    label.frame = CGRectMake(0, 0, labelWidth, 40); // 上下各10空隙
    
    if (view == nil) view = [[UIApplication sharedApplication].windows lastObject];
    UIView *showView =  [[UIView alloc]init];
    showView.backgroundColor = [UIColor blackColor];
    showView.frame = CGRectMake(0, 0, labelWidth, 40);  // 上下的间隙
    showView.center = CGPointMake(DNScreen.width /2, DNScreen.height/2);
    showView.alpha = 0.8f;
...
+ (void)noDataTipView:(UIView *)view title:(NSString *)title imageName:(NSString *)imageName  block:(void (^)())reload action:(SEL)action addTarget:(id)target {
...
+ (void)networkErrorTipView:(UIView *)view title:(NSString *)title message:(NSString *)message block:(void (^)())reload action:(SEL)action addTarget:(id)target {  // 默认图片自己更换
    [self tipView:view title:title message:message imageName:@"jiazaishibai" block:reload action:action addTarget:target];
}

#pragma mark - 内部公共方法
+ (void)refreshBtnClick:(DNRefreshButton *)btn{
...
+ (void)tipView:(UIView *)view title:(NSString *)title message:(NSString *)message imageName:(NSString *)imageName block:(void (^)())reload action:(SEL)action addTarget:(id)target {
...
- (void)tipView:(UIView *)view title:(NSString *)title message:(NSString *)message imageName:(NSString *)imageName block:(void (^)())reload action:(SEL)action addTarget:(id)target hud:(MBProgressHUD *)hud{
    
    self.hud = hud;
    CGFloat padding = 10;
    
    // 大的背景view
    CGRect bgViewFrame = CGRectMake(0, 0, DNScreen.width, DNScreen.height);
    ...
    if (reload || action) {
        ...
        DNRefreshButton *refreshBtn = [[DNRefreshButton alloc] init];
...
+ (CGFloat)widthWithFont:(UIFont *)font constrainedToHeight:(CGFloat)height text:(NSString *)text{
    ...
    textSize = [text boundingRectWithSize:CGSizeMake(CGFLOAT_MAX, height)
